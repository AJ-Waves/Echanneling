...
$error = false;
if (isset($_POST['add'])) {
    $admin_center_reg = mysqli_real_escape_string($con, $_POST['admin_center_reg']);
    ...
    if (empty($admin_center_reg)) {
        $error = true;
        $admin_center_reg_error = "Please Enter Registration Number";
    }

    if(!empty($admin_center_reg)){
        if(($sql_center_no = (mysqli_query($con,"SELECT * FROM hospital WHERE hospitalID = '$admin_center_reg'"))) && (mysqli_num_rows($sql_center_no)<=0)){
            $error = true;
            $admin_center_reg_error = "Center is not available in registered list";
        }
    }
    ...
    if (!$error) {
        ...
        if ((mysqli_query($con, "INSERT INTO center_admin(center_reg_no,user_name,admin_name,admin_email,admin_address,admin_phone) VALUES('$admin_center_reg','$admin_user_name','$admin_name','$admin_email','$admin_address','$admin_phone')")) && (mysqli_query($con, "INSERT INTO login_levels(user_name,password,user_level) VALUES('$admin_user_name','$admin_pw','2')"))) {

            header('Refresh:2; url=center_admin.php');
...
if(isset($_POST['edit'])) {
    ...
    if (!$error) {
        ...
        $update_center_admin = "UPDATE center_admin SET admin_name='$u_admin_name',admin_email='$u_admin_email',admin_address='$u_admin_address',admin_phone='$u_admin_phone' WHERE user_name='$u_user_name'";
...
if(isset($_GET['un_delete'])){
    $center_admin = $_GET['un_delete'];


    $sql_center_admin_info = "Delete FROM center_admin WHERE user_name = '$center_admin'";
    $sql_login_info = "Delete FROM login_levels WHERE user_name = '$center_admin'";

    If((mysqli_query($con,$sql_center_admin_info)) && (mysqli_query($con,$sql_login_info))){
        header('Refresh:2; url=center_admin.php');
...
<div class="col-md-8 col-xs-12 col-md-offset-1 admin-body">
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        <div class="col-md-12 row-margin">
            <h5 class="sch-h5">Update  Channelling Center Admin</h5>
        </div>

        <div class="col-md-3 col-xs-12">
            <label for="up_center_user" class="btn-label-padding">Center Reg No:</label>
        </div>

        <div class="col-md-7">
            <input type="text" id="center_id" name="center_id" class="form-control" value="<?= isset($_POST['up_center_user']) ? $_POST['up_center_user'] : ''; ?>" placeholder="Enter Center Registration Number and press Search">

        </div>
        <div class="col-md-2 col-xs-12">
            <input type="button" id="search" class="btn btn-default btn-label-padding" value="Search"/> <!--<i class="ion-ios-search"></i>-->
        </div>

        <div id="center_admin_table" class="col-md-12 row-margin">
        ...
        <div class="col-md-12 col-xs-12 row-margin">
            ...
            <div class="col-md-7 col-xs-12 row-margin">
                <input type="text" class="form-control" name="u_admin_email" value="<?php if (isset($_GET['ae'])) echo "$_GET[ae]" ?>">
            ...
            <div class="col-md-7 col-xs-12 row-margin">
                <input type="text" class="form-control" name="u_admin_name" value="<?php if (isset($_GET['an'])) echo "$_GET[an]" ?>">
            ...
            <div class="col-md-7 col-xs-12 row-margin">
                <textarea class="form-control" name="u_admin_address" rows="2"><?php if (isset($_GET['add'])) echo "$_GET[add]" ?></textarea>
            ...
            <div class="col-md-7 col-xs-12 row-margin">
                <input type="tel" class="form-control" name="u_admin_phone" value="<?php if (isset($_GET['ap'])) echo "$_GET[ap]" ?>">
        ...
        <div class="col-md-12 row-margin">
            <h5 class="sch-h5">Assigning New Channelling Center Admin</h5>
        </div>

        <div class="col-md-12 col-xs-12 row-margin">

            <div class="col-md-3 col-xs-12 row-margin">
                <label for="admin_center_reg">Center Reg No:</label>
            </div>
            <div class="col-md-7 col-xs-12 row-margin">
                <input type="text" class="form-control" name="admin_center_reg" value="<?= isset($_POST['admin_center_reg']) ? $_POST['admin_center_reg'] : ''; ?>">
                <span class="error"><?php if (isset($admin_center_reg_error)) echo $admin_center_reg_error; ?></span>
            </div>
            <div class="col-md-3 col-xs-12 row-margin">
...
<script type="text/javascript">
    $('#search').click(function(){
        ...
        $.ajax({
            url: 'get_center_admin.php',
            data: 'center_id='+center_id,
            success: function(data){
                $('#center_admin_table').html(data);
            }
        });
    });
